...
# ╔═╡ 3263b7c0-ef66-11ea-1733-69a40c3cf32b
using Images
...
student = (name = "Jihwan Jang", kerberos_id = "none")
...
function matrix_to_vecvec(matrix)
	(h, w) = size(matrix)
	return [[matrix[i,j] for j=1:w] for i=1:h]
end
...
begin
	...
	function quantize(image::AbstractMatrix)
		# you will write me in a later exercise!
		(h, w) = size(image)
		return [quantize(image[i, j]) for i=1:h, j=1:w]
	end
end
...
begin
	...
	function noisify(image::AbstractMatrix, s)
		(h, w) = size(image)
		return [noisify(image[i, j], s) for i=1:h, j=1:w]
	end
end
...
# ╔═╡ c974e52e-ef43-11ea-2e5c-038cb461eda7
decimate(philip)
...
function extend(v, i)
	n = length(v)
	return v[Int(round((n-1)*myclamp((i-1)/(n-1)))+1)]
end
...
function blur_1D(v, l)
	ext = [extend(v, i) for i=1-l:length(v)+l]
	blr = [mean(ext[i-l:i+l]) for i=l+1:length(v)+l]
	return blr
end
...
# ╔═╡ 7d79b240-ef49-11ea-3222-05284a0b8b2b
1:n
...
# ╔═╡ 4af7db30-ef49-11ea-2fe0-1ba4d4112c0a
[
	colored_line(v)	
...
function convolve_vector(v, k)
	...
	lv= length(v)
	l = (lk-1)÷2
	ext = [extend(v, i) for i=1-l:lv+l]
	con = [sum([ext[i+j-l]*k[lk-j] for j=0:lk-1]) for i=1+l:l+lv]
	return con
...
# ╔═╡ 19f70530-ef51-11ea-11cc-eb85051f051d
asdf = [1.,2.,3.]

# ╔═╡ 2037f8f2-ef51-11ea-2beb-2944699ed46e
asdf ./= sum(asdf)

# ╔═╡ 31a57d60-ef51-11ea-1995-7d9b4b19506b
sum(asdf)

# ╔═╡ 1c8b4658-ee0c-11ea-2ede-9b9ed7d3125e
function gaussian_kernel(n)
	nn = (n-1)÷2
	lst = [exp(-i^2/2) for i=-nn:nn]
	lst ./= sum(lst)
	return lst
end
...
# ╔═╡ 51e844a0-ef50-11ea-292f-3d0560171a76
vv = random_vect

# ╔═╡ 96a4db30-ef50-11ea-3083-f96ddea3ce91
@bind sz Slider(1:2:9, default=3, show_value=true)

# ╔═╡ 7644482e-ef50-11ea-0c18-537133c9b47b
[
	colored_line(vv)
	colored_line(convolve_vector(vv, gaussian_kernel(sz)))
]

# ╔═╡ 38eb92f6-ee13-11ea-14d7-a503ac04302e
test_gauss_1D_a = let
	k = gaussian_kernel(sz)
...
# 		end
# 	end
	vec = matrix_to_vecvec(M)
	return extend(extend(vec, i), j)
end
...
# ╔═╡ 50070b0e-ef5c-11ea-19f8-ab0edf953544
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	hk, wk = size(K)
	lh, lw = (hk-1)÷2, (wk-1)÷2
	ext = [extend_mat(M, i, j) for i=1-lh:hm+lh, j=1-lw:wm+lw]
	con = [sum([ext[z+x-lh, w+y-lw]*K[hk-x, wk-y] for x=0:hk-1, y=0:wk-1]) for z=1+lh:lh+hm, w=1+lw:lw+wm]
	return con
end
...
# ╔═╡ f77b1900-ef60-11ea-0bec-0f69b24cb030
function gaussian_matrix(n)
	nn = (n-1)÷2
	lst = [exp(-(i^2+j^2)/2/n^2) for i=-nn:nn, j=-nn:nn]
	lst ./= sum(lst)
	return lst
end	

# ╔═╡ e20906ce-ef61-11ea-07f5-c12cec76712f
gaussian_matrix(3)

# ╔═╡ 2c4a6d10-ef62-11ea-2492-5b5ffb7bd5b2
@bind blur_ Slider(1:2:20, default=3, show_value=true)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image) 
	return convolve_image(image, gaussian_matrix(blur_))
end
...
# ╔═╡ e6bfa250-ef67-11ea-1ac0-cf5723ddc4e7
Gx = [
	1 0 -1
	2 0 -2
	1 0 -1
...
# ╔═╡ f346e100-ef67-11ea-0d76-8bee3d54d8bc
Gy = [
	1 2 1
	0 0 0
	-1 -2 -1
...
function with_sobel_edge_detect(image)
	Sobel_x = convolve_image(image, Gx)
	Sobel_y = convolve_image(image, Gy)
	h, w = size(image)
	return [Gray(sqrt(Sobel_x[i, j]^2+Sobel_y[i, j]^2)) for i=1:h, j=1:w]
end

# ╔═╡ 2a1d5042-efd8-11ea-1420-4963959f37ae
md"""# Final Summary"""

# ╔═╡ 6d6fb2fe-efd9-11ea-02df-09052e7378ab
begin
	function GsnBlr(img, n)
		return convolve_image(img, gaussian_matrix(n))
	end
	
	function SblEdgDtc(img; extreme=false, pivot=0.5)
		image = Gray.(img)
		Sobel_x = convolve_image(image, Gx)
		Sobel_y = convolve_image(image, Gy)
		h, w = size(image)
		Dtcn = [sqrt(Sobel_x[i, j]^2+Sobel_y[i, j]^2) for i=1:h, j=1:w]
		if extreme
			return [Gray(Dtcn[i, j]>pivot ? 1 : 0) for i=1:h, j=1:w]
		else
			return Gray.(Dtcn)
		end
	end
...
# ╔═╡ 47e1b492-efd8-11ea-3d9a-a7ff5febfe54
md""" ### 1. Loading Images"""

# ╔═╡ 622a5fa0-efd8-11ea-0fea-11b365546119
img = decimate(load("G:/My Drive/SNU_ECE/Julia/WIN_20200906_09_32_51_Pro.jpg"),4);

# ╔═╡ e2178cb0-efd8-11ea-01e8-21d20824c994
@bind img_blur Slider(1:2:9, default=5, show_value=true)

# ╔═╡ d0d488c0-efda-11ea-09cc-098a1e1e1f98
@bind img_edge Slider(0:0.05:1, default=0.5, show_value=true)

# ╔═╡ e5768440-efda-11ea-30e2-e7aa1975006a
[
	img GsnBlr(img, img_blur)
	SblEdgDtc(img) SblEdgDtc(img, extreme=true, pivot=img_edge)
]

# ╔═╡ df6376f0-efdd-11ea-35e7-fdf04b39b143
md""" ### 2. Taking Photos"""

# ╔═╡ 2ab14420-efde-11ea-0d97-5ba4949f50da
@bind pic_blur Slider(1:2:9, default=3, show_value=true)

# ╔═╡ 5c8acd40-efde-11ea-1eeb-4563e5561e84
@bind pic_edge Slider(0:0.05:1, default=.5, show_value=true)
...
# ╔═╡ b83ccbd0-ef47-11ea-0851-31908613c31b
if !@isdefined(extend)
	not_defined(:extend)
else
	let
...
@bind gauss_raw_camera_data camera_input(;max_size=500)

# ╔═╡ 1a0324de-ee19-11ea-1d4d-db37f4136ad3
@bind sobel_raw_camera_data camera_input(;max_size=2000)

# ╔═╡ cd2aa4f0-ef69-11ea-1628-df4d211555e9
@bind final_cam camera_input(;max_size=1000)

# ╔═╡ fef5e480-efdd-11ea-0541-ef94be6eb00f
@bind raw_pic camera_input(;max_size=1000)
...
# ╔═╡ b68f18d0-ef68-11ea-075f-3f50af35f802
size(sobel_camera_image)

# ╔═╡ 1bf94c00-ee19-11ea-0e3c-e12bc68d8e28
with_sobel_edge_detect(sobel_camera_image)

# ╔═╡ 8bad8c80-ef6a-11ea-0178-bfc331da9c24
fin_pic = process_raw_camera_data(final_cam);

# ╔═╡ dbf1cc10-ef6a-11ea-3a8c-3df59cd04a4d
typeof(fin_pic)

# ╔═╡ b558f7e2-ef6a-11ea-273c-5993c3f8217e
fin_pic_sbl = Gray.(fin_pic);

# ╔═╡ 42b03f00-ef6a-11ea-0380-b12646fcdb15
[
	fin_pic noisify(fin_pic, 1.)
	with_gaussian_blur(fin_pic) with_sobel_edge_detect(fin_pic_sbl)
]

# ╔═╡ ec94ff60-efdd-11ea-06c6-cd16acada0b5
pic = process_raw_camera_data(raw_pic)

# ╔═╡ 19055720-efde-11ea-2280-a3f9da1f669d
[
	pic GsnBlr(pic, pic_blur)
	SblEdgDtc(pic) SblEdgDtc(pic, extreme=true, pivot=pic_edge)
]
